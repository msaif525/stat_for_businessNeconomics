...
**Note:** The sample correlation coefficient can be expressed in other forms:
...
Where, $z_x$ and $z_y$ are the z-score of $X$ and $Y$ respectively.

## Properties of coefficient of correlation

1.  The value of $\rho$ is always between -1 and 1 inclusive. That is $-1\le \rho\le+1$.

2.  If all values of either variable are converted to a different scale, the value of $\rho$ does not change (NOT affected by change of *origin* and *scale*).

3.  The value of $\rho$ is not affected by the choice of X or Y. Interchange all x values and y values, and the value of $\rho$ will not change ($\rho$ is a symmetric measure).

4.  $\rho$ measures the strength of a *linear relationship*. It is not designed to measure the strength of a relationship that is not linear.

5.  $\rho$ is very sensitive to outliers in the sense that a single outlier could dramatically affect its value.
...
::: callout-note
## Effect of change of origin and scale on $\rho$

Let, $U=\frac{X-a}{b}$ and $V=\frac{Y-c}{d}$. Here $a,b,c,d$ are any arbitrary real numbers.
...
Now, the **residuals sum of square (RSS)** is given as:

$$RSS=\sum_{i=1}^n e_i^2=\sum_{i=1}^n (y_i-\hat y_i)^2=\sum_{i=1}^n (y_i-\hat \beta_0-\hat \beta_1x_i)^2$$
 {#eq-ssr}

In **OLS** method we *minimize* the **RSS**with respect to $\hat \beta_0$ and $\hat \beta_1$. To minimize **RSS**we have to set two equations using *partial derivative:*

$$\frac{\delta \hspace{1pt} (RSS)}{\delta \hat \beta_0}=0$$
 {#eq-ols1}

$$\frac{\delta \hspace{1pt} (RSS)}{\delta \hat \beta_1}=0$$
 {#eq-ols2}
...
In the case of **SLR**, SSR can be written as $$SSR=\hat \beta_1^2\sum_{i=1}^n (x_i-\bar x)^2=\hat \beta_1^2 (n-1)s_x^2$$
...
**Problem 15.4** **Age and Blood Pressure.** The ages (in years) of 10 men and their systolic blood pressures (in millimeters of mercury).

|                                   |     |     |     |     |     |     |     |     |     |     |
|-------|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|:-----:|-------|
| **Age**                           | 16  | 25  | 39  | 45  | 49  | 64  | 70  | 29  | 57  | 22  |
| **Systolic blood pressure (SBP)** | 109 | 128 | 143 | 145 | 180 | 185 | 185 | 199 | 175 | 118 |
